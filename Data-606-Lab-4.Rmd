...
sd(fdims$hgt)
```
*Both male and female height sets follow a normal distribution.  Male heights have an mean of 177.75cm with a standard devation of 7.18cm, and female heights have an average of 164.87cm with a standard devation of 6.54.*
...
qqline(sim_norm)
```
*Although not all the points fall on the line, the points fall close to the line.  The plot compares well to the plot for the real data.  Thus, it is likely that the real data follows a normal distribution*
...
*The normal probability plot for fdims$hgt does look similar to these plots.  They provide visual evidence that the female heights follow a nearly normal distribution.*
...
*What is the proabibility that a randomly chosen female shorter than 155 cm?*
```{r}
#Theoretical:
...
*What is the proabibility that a randomly chosen female heavier than 65kg?*
```{r}
#Setup
...
*Age is reported to the year, so values won't be continuous (e.g. no 25.87 years; would be 26)*
*Elbow width is is reported to the nearest 10th, and it's also a fairly small number, so it will also staircase.*
