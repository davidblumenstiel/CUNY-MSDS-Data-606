...
*This is likely a normal distribution.  Although there is a theoreical limit to the lowest space in homes, and no limit to max, when we look at the data summary we can see that the mean and median are fairly close, and that median straddles the IQR evenly.  The mean is 1500, and the stdev is 506.*
...
sd(samp1)
```
*The distribution is similar, although much of the clarity is lost.  The mean is 1535, and the stdev is 529.*
...
sd(samp2)
```
*samp2 is also similar to samp1.  The mean of samp 2 is 1519, and the stdev is 434.  If we were to take more samples with larger sizes, the one with the highest size (1000 in the case above) would be the most representative of the true mean.*
...
*There are 27 bins within the histogram, and 5000 observations.  The distrubiton is normal with a mean of 1502, and an stdev of 71.  I would not expect the distribution to change much if we collected 50,000 samples; although, it may become even more normal.*
...
for(i in 1:5000){
   samp <- sample(area, 50)
   sample_means50[i] <- mean(samp)
   #print(i)  Not going to print this in the knit.
   }
```
...
sample_means_small
```
*There are 100 elements in sample_means_small.  Each element represents the mean of 50 random samples of 'area'.*
...
*The sample tends to stay the same, but as the sample size increases, the spread tightens.*
...
```{r}
set.seed("1234567890")
psam <- sample(price,50)
mean(psam)
```
*For this seed, it's 194576.2*
...
sample_means50 <- rep(NA,5000)
for(i in 1:5000){
   samps = sample(price, 50)
   sample_means50[i] <- mean(samps)
}
hist(sample_means50)
mean(sample_means50)
sd(sample_means50)
mean(price)
```
*This is a normal distribution with a mean of 181019 and a stdev of 11137.  Based on the distribution one would expect a mean home price around 180000.  The mean home price based on this population is 181019.*
...
for(i in 1:5000){
   samps = sample(price, 150)
   sample_means150[i] <- mean(samps)
}
...
```
*This is a normal distribution with a mean of 180817 and a stdev of 6366.  Compared to a sample size of 50, the mean is similiar and the stdev and IQR are lower.  Based on this sampling distribution, we wouldguess the mean sale price to be 180817.*
...
*The 'sample_means150' has a smaller spread.  We would perfer a distribution with a small spread*
