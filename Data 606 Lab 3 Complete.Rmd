...
#I could not get the file to open this time.  The code below will need to be changed for repdocutability.

load("C:\\Users\\blume\\Desktop\\CUNY MSDS\\Data 606 Statistics and Probability for Data Analytics\\Lab3\\more\\kobe.RData")
head(kobe)
```
...
*A streak length of 1 means one hit, a streak length of 0 means no hits*
...
*This is a right skewed distribution.  His typical streak was 0, the standard deviation is 0.99, while 4 was his maximum streak.*
...
```
*28 this time, but 20 on average*
...
```
*The probability of making a hit needed to be adjusted to 45%, an the sample size needed to be ammended to 133.*
...
```
*This is a right skewed distribution, with a median of 0, and a standard deviation of 1.26.  The max streak was 5.*
...
*I would expect it to be similarly shaped (right skewed, mode of 0) as it has a fair sample size and is based on probability, but not the same as it's random what each individual result will be, and thus unlikely to be exactly the same.*   
...
*OVerall it's pretty simmilar; the simulation even returned higher streaks in the current runthrough.  This indicates that Kobe's shots were likely independant of eachother.*
