...
*There are 20000 cases in this dataset, and 9 variables.   genhlth, exerany, hlthplan, smoke100, and gender are all categorical data types; height, wieght, wtdesire, and age are all reported as discrete.*
...
```
```{r}
#Proportion reporting exce'lent health
...
*A higher proportion of smokers are men than women*
...
*the boxplot above shows bmi vs genhlth.  It shows those who report better health status tend to be of lower bmi.  Below is a boxplot of bmi against exerany.  It stands to reason that exercise might be correlated with a lower bmi; it does indeed suggest this.*
```{r}
...
```
    *Those with higher weights tend to desire lower weights near to their current weight.*
...
*wdiff is discrete data,  If wdiff == 0 then the person in question does not wish to change their weight.  If wdiff is positive then they wish to loose weight; if it is negative they wish to gain weight.*
...
boxplot(wdiff)
```
*wdiff is centered around 10 lbs (median) of desired wieght loss, with a first quartile of 0 and a 3rd quartile of 21.  It has a fairly tight spread (mean == 14.59), with outliers tending to occur among those wanting to loose weight.  This tells us that most people would like to loose about 10 lbs of weight.*
...
median(wdiff[cdc$gender == "m"])
median(wdiff[cdc$gender == "f"])
```
    *Men tend to want to loose a median of 5 lbs while women want to loose 10 lbs*
...
```{r}
#Proportion of observations with weights within one stdev
weight1sd <- subset(cdc, weight < wmean + wsd & weight > wmean - wsd)
nrow(weight1sd)/20000
```
