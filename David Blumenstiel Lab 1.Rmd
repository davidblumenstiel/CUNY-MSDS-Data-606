---
title: "Data 606 Lab 1: Introduction to R and RStudio"
output:
...
*There is a general upwards trend over time, but it dips beteen 1640 and 1660 (English Civil War?).*
...
*There are thousands times more children being baptized in the present dataset*
```{r}
(present$boys + present$girls)/arbuthnot$boys + arbuthnot$girls
```
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
*There are more boys being born than girls, although the trend appeaers to be declining slightly over time*
```{r}
...
```{r}
maxbirths <- max(present$boys + present$girls)
maxyear <- present$year[present$boys + present$girls == maxbirths]
